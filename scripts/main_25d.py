...
def predict(args):
    ...
    # valid_data_loader = get_dataset(args, 'valid')
    valid_transform = get_validation_augmentation(args)
    # ============ building Clusformer ... ============

    model = get_model(args, distributed=False)
    model.eval()
    ...
    colors = []
    cnt = 0
    with open("color.txt") as reader:
        for line in reader:
            x, y, z = line.split()
            colors += ([int(x), int(y), int(z)])

    with open('data/Auto-retail-syndata-release/labels.txt') as f:
        labels = f.readlines()
        names = [label.strip().split(',')[0] for label in labels]
        ids = [label.strip().split(',')[1] for label in labels]
        ids = [int(x) for x in ids]

    id_to_name = {
        k: v for k, v in zip(ids, names)
    }
    ...
    def colorize_mask(mask):
        ...
        new_mask.putpalette(colors)
        return np.asarray(new_mask.convert("RGB"))

    def blend_image(image, mask):
        mask = colorize_mask(mask)
        image = image * alpha + (1 - alpha) * mask
        image = np.clip(image, 0, 255).astype(np.uint8)
        return image

    def infer_video(video):
        if not os.path.isfile(video):
            print(f"{video} is not a file")
        ...
        rxmin, rymin, rxmax, rymax = [
            597.9111328125, 304.0654296875, 1359.9672241210938, 895.7943725585938]
        rxmin = int(rxmin)
        rymin = int(rymin)
        rxmax = int(rxmax)
        rymax = int(rymax)
        ...
        while True:
            ...
            # frame = frame[rymin:rymax, rxmin:rxmax]
            org_h, org_w = frame.shape[:2]
            org_h = org_h // 2
            org_w = org_w // 2
            save_frame = frame.copy()
            save_frame = cv2.resize(save_frame, (org_w, org_h))
            frame = valid_transform(image=frame)['image']
            frame = frame.transpose((2, 0, 1)).astype(np.float32)
            frame = np.expand_dims(frame, 0)
            frame = torch.from_numpy(frame).cuda()
            with torch.no_grad():
                logits = model(frame)
            masks = logits.argmax(1)
            for i, (image, mask) in enumerate(zip(frame, masks)):
                mask = mask.detach().cpu().numpy()
                mask = mask.astype(np.uint8)
                # import pdb; pdb.set_trace()
                mask = cv2.resize(mask, (org_w, org_h))
                pred_ids = np.unique(mask)
                pred_names = []
                for cls_id in pred_ids:
                    if cls_id == 0:
                        continue

                    pred_name = id_to_name[cls_id]
                    pred_names.append(pred_name)

                pred_names = "|".join(pred_names)
                image = blend_image(save_frame, mask)
                cv2.putText(image, pred_names, (10, 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)

                hm_h, hm_w = image.shape[:2]
                if vid_writer is None:
                    vid_writer = cv2.VideoWriter(
                        f"{args.output_dir}/{video_name}.avi",

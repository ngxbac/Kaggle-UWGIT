...
class MBConvBlock(nn.Module):
    def __init__(
        ...
        if self.expand_ratio != 1:
            self._expand_conv = conv_type(
                in_channels=inp, out_channels=oup, kernel_size=1, bias=False)
            self._expand_conv_padding = _make_same_padder(
                self._expand_conv, image_size)

            self._bn0 = get_norm_layer(
                name=norm, spatial_dims=spatial_dims, channels=oup)
        else:
        ...
        self._project_conv = conv_type(
            in_channels=oup, out_channels=final_oup, kernel_size=1, bias=False)
        self._project_conv_padding = _make_same_padder(
            self._project_conv, image_size)
        self._bn2 = get_norm_layer(
            name=norm, spatial_dims=spatial_dims, channels=final_oup)
    ...
    def set_swish(self, memory_efficient: bool = True) -> None:
        ...
        self._swish = Act["memswish"](
            inplace=True) if memory_efficient else Act["swish"](alpha=1.0)
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        self._conv_stem = conv_type(
            self.in_channels, out_channels, kernel_size=3, stride=stride, bias=False)
        self._conv_stem_padding = _make_same_padder(
            self._conv_stem, current_image_size)
        self._bn0 = get_norm_layer(
        ...
        for idx, block_args in enumerate(self._blocks_args):
            block_args = block_args._replace(
                input_filters=_round_filters(
                    block_args.input_filters, width_coefficient, depth_divisor),
                output_filters=_round_filters(
                    block_args.output_filters, width_coefficient, depth_divisor),
                num_repeat=_round_repeats(
                    block_args.num_repeat, depth_coefficient),
            )
        ...
        self._conv_head = conv_type(
            head_in_channels, out_channels, kernel_size=1, bias=False)
        self._conv_head_padding = _make_same_padder(
            self._conv_head, current_image_size)
        self._bn1 = get_norm_layer(
            name=norm, spatial_dims=spatial_dims, channels=out_channels)
    ...
    def set_swish(self, memory_efficient: bool = True) -> None:
        ...
        """
        self._swish = Act["memswish"](
        ) if memory_efficient else Act["swish"](alpha=1.0)
    ...
    def _initialize_weights(self) -> None:
        ...
        for _, m in self.named_modules():
            if isinstance(m, (nn.Conv1d, nn.Conv2d, nn.Conv3d)):
                fan_out = reduce(operator.mul, m.kernel_size,
                                 1) * m.out_channels
...
class EfficientNetBN(EfficientNet):
    def __init__(
        ...
        weight_coeff, depth_coeff, image_size, dropout_rate, dropconnect_rate = efficientnet_params[
            model_name]
...
def drop_connect(inputs: torch.Tensor, p: float, training: bool) -> torch.Tensor:
    ...
    random_tensor: torch.Tensor = torch.rand(
        random_tensor_shape, dtype=inputs.dtype, device=inputs.device)
...
def _get_same_padding_conv_nd(
    ...
    _paddings: List[Tuple[int, int]] = [
        (_p // 2, _p - _p // 2) for _p in _pad_size]
    ...
    _paddings_ret: List[int] = [
        outer for inner in reversed(_paddings) for outer in inner]
...
def _make_same_padder(conv_op: Union[nn.Conv1d, nn.Conv2d, nn.Conv3d], image_size: List[int]):
    ...
    padding: List[int] = _get_same_padding_conv_nd(
        image_size, conv_op.kernel_size, conv_op.dilation, conv_op.stride)
...
def _round_filters(filters: int, width_coefficient: Optional[float], depth_divisor: float) -> int:
    ...
    new_filters: float = max(divisor, int(
        filters_float + divisor / 2) // divisor * divisor)
